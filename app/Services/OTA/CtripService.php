...
class CtripService
{
    ...
    public function syncProductStockByCombo(
        ...
        foreach ($inventoryList as $inventory) {
            ...
            if (isset($closedControlDates[$date])) {
                $inventoryByDate[$date]['quantity'] = 0;
                $inventoryByDate[$date]['is_closed'] = true;
        ...
        if ($stayDays && $stayDays > 1) {
            ...
            foreach ($inventoryByDate as $date => $data) {
                ...
                for ($i = 0; $i < $stayDays; $i++) {
                    ...
                    if (! isset($inventoryByDate[$checkDate])) {
                        $missingInventory = $allDateInventories->get($checkDate);
                        ...
                        // 如果不在销售日期范围内、关闭或库存为0、产品维度关闭，无法满足连续入住
                        if (
                            ! $isInSalePeriod
                            || $missingInventory->is_closed
                            || $missingInventory->available_quantity <= 0
                            || isset($closedControlDates[$checkDate])
                        ) {
                            $canAccommodate = false;
                            $checkDetails[] = [
                                'check_date' => $checkDate,
                                ...
                                'reason' => ! $isInSalePeriod
                                    ? '不在销售日期范围内'
                                    : (isset($closedControlDates[$checkDate]) ? '产品维度已关闭' : ($missingInventory->is_closed ? '库存已关闭' : '库存为0')),
                            ];
