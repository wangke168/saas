...
class TicketPolicy
{
    ...
    {
        // 所有已认证用户都可以查看门票列表（权限控制在 Controller 中通过查询过滤实现）
        return true;
    }
    ...
    public function view(User $user, Ticket $ticket): bool
    {
        ...
        if ($user->isOperator()) {
            ...
            return $scenicSpotIds->contains($ticket->scenic_spot_id);
        }
     ...
     */
    public function update(User $user, Ticket $ticket, $newScenicSpotId = null): bool
    {
        // 超级管理员可以更新所有门票
        if ($user->isAdmin()) {
            return true;
        ...
        if ($user->isOperator()) {
            ...
            // 如果修改了景区，检查新景区的权限
            if ($newScenicSpotId !== null && $newScenicSpotId != $ticket->scenic_spot_id) {
