...
class TestCtripConfirmOrder extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'test:ctrip-confirm-order 
                            {--order= : 使用订单ID测试}
                            ...
                            {--voucher-sender= : 凭证发送方（默认：1，携程发送）}
                            {--json : 以JSON格式输出}
     ...
     * @var string
     */
    protected $description = '测试携程订单确认接口 - CreateOrderConfirm';
    ...
    {
        $this->info('========================================');
        $this->info('携程订单确认接口测试 - CreateOrderConfirm');
        $this->info('========================================');
        $this->newLine();
    ...
    protected function testWithOrderId(int $orderId): int
    {
        ...
        // 调用携程服务
        $this->info("[开始测试]");
        $this->line("调用 CtripService::confirmOrder()...");
        $this->newLine();
    ...
    protected function testWithManualInput(): int
    {
        ...
        $voucherSender = (int)($this->option('voucher-sender') ?: 1);
        ...
        $this->line("  凭证发送方: {$voucherSender} (1=携程发送, 2=供应商发送)");
        $this->newLine();
        ...
        // 调用携程客户端
        $this->info("[开始测试]");
        $this->line("调用 CtripClient::confirmOrder()...");
        $this->newLine();
    ...
    protected function diagnoseError(array $result, ?Order $order = null, ?array $manualData = null): void
    {
        ...
        if (strpos($errorMessage, 'serviceName') !== false || strpos($errorMessage, 'ServiceName') !== false) {
            ...
            $this->newLine();
            $this->line("  可能原因:");
            $this->line("    1. serviceName 应该是 'CreateOrderConfirm'");
            ...
            $this->newLine();
            $this->line("  可能原因:");
            $this->line("    1. URL 不正确，应该是 CreateOrderConfirm.do 接口");
            ...
            $this->line("    2. 检查日志中的 '携程订单确认请求数据' 记录");
