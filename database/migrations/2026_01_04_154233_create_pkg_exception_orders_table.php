...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('pkg_exception_orders')) {
            return;
        }
        
        Schema::create('pkg_exception_orders', function (Blueprint $table) {
            $table->id();
            $table->foreignId('order_id')->constrained('pkg_orders')->onDelete('cascade')->comment('订单ID');
            ...
            $table->index('exception_type', 'idx_exception_type');
            $table->index('handler_id', 'idx_handler');
        });
    }
    ...
    {
        Schema::dropIfExists('pkg_exception_orders');
    }
};
