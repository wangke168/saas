...
// 1. 检查配置（.env 经 config/services.php，兼容 php artisan config:cache）
echo "1. 检查配置 (services.meituan)...\n";
$configs = [
    'MEITUAN_PARTNER_ID' => config('services.meituan.partner_id'),
    'MEITUAN_APP_KEY' => config('services.meituan.app_key'),
    'MEITUAN_APP_SECRET' => config('services.meituan.app_secret'),
    'MEITUAN_AES_KEY' => config('services.meituan.aes_key'),
    'MEITUAN_API_URL' => config('services.meituan.api_url'),
    'MEITUAN_WEBHOOK_URL' => config('services.meituan.webhook_url'),
];
...
foreach ($configs as $key => $value) {
    if (empty($value)) {
        ...
        if (in_array($key, ['MEITUAN_APP_SECRET', 'MEITUAN_AES_KEY'])) {
            $displayValue = substr((string) $value, 0, 8) . '...' . substr((string) $value, -4);
        } else {
...
if (!$allValid) {
    echo "\n❌ 部分配置未设置，请检查 .env 并执行 php artisan config:cache\n";
    exit(1);
}
echo "\n";
...
$aesKey = (string) config('services.meituan.aes_key', '');
if (strlen($aesKey) !== 16) {
...
$webhookUrl = (string) config('services.meituan.webhook_url', '');
if (empty($webhookUrl)) {
...
try {
    ...
    $config->account = config('services.meituan.partner_id');
    $config->secret_key = config('services.meituan.app_key');
    $config->aes_key = config('services.meituan.app_secret');
    $config->aes_iv = config('services.meituan.aes_key');
    $config->api_url = config('services.meituan.api_url');
...
// 6. 生成完整接口地址列表
echo "6. 生成完整接口地址列表...\n";
$baseUrl = rtrim((string) config('services.meituan.webhook_url', ''), '/');
$interfaces = [
