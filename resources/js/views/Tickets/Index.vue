    ...
    <div>
        ...
        <el-card>
            <div style="margin-bottom: 20px;">
                <el-button type="primary" @click="handleCreate">创建门票</el-button>
                <el-select
            ...
            <el-table :data="tickets" v-loading="loading" border>
                <el-table-column prop="name" label="门票名称" width="200" />
                <el-table-column prop="code" label="门票编码" width="150" />
                <el-table-column prop="external_ticket_id" label="外部门票编号" width="150">
        ...
        >
            ...
            >
                ...
                <el-form-item label="门票编码" prop="code">
                    <el-input v-model="form.code" placeholder="系统自动生成" :disabled="true" />
                    <div style="font-size: 12px; color: #909399; margin-top: 5px;">
                        门票编码由系统自动生成，格式：T + 5位数字（如 T00001）
                    </div>
                </el-form-item>
                <el-form-item label="外部门票编号" prop="external_ticket_id">
                    <el-input v-model="form.external_ticket_id" placeholder="用于资源方系统对接的门票编号（可选）" />
                </el-form-item>
...
const dialogTitle = computed(() => isEdit.value ? '编辑门票' : '创建门票');
...
const rules = {
    ...
    name: [
        ...
        { max: 255, message: '门票名称不能超过255个字符', trigger: 'blur' }
    ],
};
...
const handleSubmit = async () => {
    ...
    await formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                if (isEdit.value) {
                    await axios.put(`/tickets/${editingId.value}`, form.value);
                    ElMessage.success('门票更新成功');
                } else {
...
const handleDelete = async (row) => {
    try {
        ...
        await axios.delete(`/tickets/${row.id}`);
